...
- `averbacao.service.js` - Lógica de averbações (consolidação 1 empresa → N containers por período)
- `seguradora.service.js` - Lógica de seguradoras
- `permissao.service.js` - Lógica de permissões e controle de acesso

> **Fluxo consolidado de averba��es:** `averbacao.service.js` exige `clienteId`, `periodoInicio` e `periodoFim` para gerar uma averba��o �nica por empresa. O servi�o agrega todas as viagens (`containerTrip`) do intervalo, persiste os v�nculos na tabela piv� `averbacao_container`, recalcula import�ncia segurada/pr�mios e valida `containerTripIds` informados pelo front (pertencimento ao cliente + ader�ncia ao per�odo).
...
> **Persist�ncia multi-container:** `averbacao.repository.js` exp�e helpers (`attachContainers`/`detachContainers`) que gravam na tabela piv� `averbacao_container` e incluem automaticamente as viagens (`containerTrip`) relacionadas em todas as consultas.
